...
for block in range (n_blocks): #range(n_blocks) (0, 1, 2, 3, 4, 5)
    ...
    for trial in range(n_trials_block): #vier trials pro block
        ...
        elif trial > 1 and trial <= 3: #wie ifelse() in R?
            trialtype = 'incongruent'
        elif trial >= 4 and trial < 6:
            trialtype = 'neutral'
        if curr_stim[3] == '←':
            correctresponse = 120
        elif curr_stim[3] == '→':
            correctresponse = 109
        elif curr_stim[3] == '↑' or curr_stim[3] == '↓':
            correctresponse = None #neutraler Stimulus, keine richtige Antwort
